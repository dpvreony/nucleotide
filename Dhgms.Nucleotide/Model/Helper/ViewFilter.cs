...
namespace Dhgms.Nucleotide.Model.Helper
{
    ...
    public class ViewFilter : BaseClassGenerator
    {
        ...
        {
            sb.AppendLine(
                "            " + pi.GetCSharpDataTypeDeclaration() + " " + Common.GetVariableName(pi.Name) + " = "
                + pi.DefaultValue + ";");
            sb.AppendLine("            if(" + Common.GetVariableName(pi.Name) + "Ordinal > -1)");
            sb.AppendLine("            {");
            sb.AppendLine(
                "                " + Common.GetVariableName(pi.Name) + " = dataReader.Get" + pi.SqlDataReaderType + "("
            ...
            StringBuilder sb, string className, Base[] properties, string baseClassName, Base[] baseClassProperties)
        {
            ...
            string modifier = (!string.IsNullOrWhiteSpace(baseClassName) && baseClassProperties != null
                               && baseClassProperties.Length > 0)
                                  ? "new "
                                  : "override ";
            sb.AppendLine("        public " + modifier + className + this.GetClassSuffix() + " GetStronglyTypedObjectFromDataReaderRow(System.Data.Common.DbDataReader dataReader)");
            sb.AppendLine("        {");
            ...
            if (baseClassProperties != null && baseClassProperties.Length > 0)
            {
                ...
                {
                    sb.AppendLine("                ," + Common.GetVariableName(baseClassProperties[i].Name));
                }
            }
            ...
            {
                sb.AppendLine("                ," + Common.GetVariableName(properties[i].Name));
            }
