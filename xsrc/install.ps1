param($installPath, $toolsPath, $package, $project)
    ...
    $target = $msbuild.Xml.AddTarget("EnsureNucleotideImported")
    $target.BeforeTargets = "BeforeBuild"
    $target.Condition = "'`$(NucleotideImported)' == ''"

    # if the targets don't exist at the time the target runs, package restore didn't run
    $errorTask = $target.AddTask("Error")
    $errorTask.Condition = "!Exists('$relativePath') And ('`$(RunNucleotide)' != '' And `$(RunNucleotide))"
    $errorTask.SetParameter("Text", "You are trying to build with Nucleotide, but the NuGet targets file that Nucleotide depends on is not available on this computer. This is probably because the Nucleotide package has not been committed to source control, or NuGet Package Restore is not enabled. Please enable NuGet Package Restore to download them. For more information, see http://go.microsoft.com/fwlink/?LinkID=317567.");
    ...
    $errorTask = $target.AddTask("Error")
    $errorTask.Condition = "Exists('$relativePath') And ('`$(RunNucleotide)' != '' And `$(RunNucleotide))"
    $errorTask.SetParameter("Text", "Nucleotide cannot be run because NuGet packages were restored prior to the build running, and the targets file was unavailable when the build started. Please build the project again to include these packages in the build. You may also need to make sure that your build server does not delete packages prior to each build. For more information, see http://go.microsoft.com/fwlink/?LinkID=317568.");
